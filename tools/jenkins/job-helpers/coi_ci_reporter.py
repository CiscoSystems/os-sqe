...
NEUTRON_GITHUB_URL = {"api": "https://api.github.com/repos/openstack/neutron/git/refs/heads/master", "url": "https://github.com/openstack/neutron"}
INTERNAL_NEUTRON_GITHUB_URL = {"api": "https://api.github.com/repos/cisco-openstack/neutron/git/refs/heads/staging", "url": "https://github.com/cisco-openstack/neutron"}
TEMPEST_GITHUB_URL = {"api": "https://api.github.com/repos/CiscoSystems/tempest/git/refs/heads/production", "url": "https://github.com/CiscoSystems/tempest"}
GITHUB_URLS = {0: {"api": "https://api.github.com/repos/openstack/neutron/git/refs/heads/master?client_id=ebagdasa&client_secret=dd083d7427353c1d410a4d069e2d895d5b7e641d", "url": "https://github.com/openstack/neutron"},
3: {"api": "https://api.github.com/repos/openstack-dev/devstack/git/refs/heads/master?client_id=ebagdasa&client_secret=dd083d7427353c1d410a4d069e2d895d5b7e641d", "url": "https://github.com/openstack-dev/devstack"},
4: {"api": "https://api.github.com/repos/CiscoSystems/tempest/git/refs/heads/production?client_id=ebagdasa&client_secret=dd083d7427353c1d410a4d069e2d895d5b7e641d", "url": "https://github.com/CiscoSystems/tempest"}
}
...
def get_failed_tests(data):
    ...
    for job in data:
        ...
        if "results_link" not in data[job] or job == "total":
            continue
        json_test_results = requests.get(data[job]["results_link"] + "api/json?pretty=true").json()
        failed_tests_amount = 0
        for test_case in json_test_results["suites"][0]["cases"]:
            bugno = ""
            bug_state = ""
            for bug in bug_list:
                if (test_case["className"] + "." + test_case["name"]) in bug["className"]:
                    bugno = bug["bugNo"]
                    bug_state = launchpad.bugs[bugno].bug_tasks[0].status
            if test_case['status'] == "REGRESSION":
                if bugno:
                    regressed_test_cases.append(test_case["className"] + "." + test_case["name"] + '   -   unstable bug <a href="https://bugs.launchpad.net/tempest/+bug/{bugNo}">{bugNo}</a>'.format(bugNo=bugno))
                else:
                    regressed_test_cases.append(test_case["className"] + "." + test_case["name"] + '   -  under investigation')
            if test_case['status'] == "FIXED":
                if bugno:
                    fixed_test_cases.append(test_case["className"] + "." + test_case["name"]+  '   -   unstable bug <a href="https://bugs.launchpad.net/tempest/+bug/{bugNo}">{bugNo}</a>'.format(bugNo=bugno))
                else:
                    fixed_test_cases.append(test_case["className"] + "." + test_case["name"] + '   -  under investigation')
            if test_case['status'] == "REGRESSION" or test_case['status'] == "FAILED":
                failed_tests_amount += 1
                bug_status = "under investigation"
                if bugno:
                    bug_status = '<a href="https://bugs.launchpad.net/tempest/+bug/{bugNo}">{bugNo}</a>  -  {bug_state}'.format(bugNo=bugno, bug_state=bug_state)
                failed_tests.append({"test_no": failed_tests_amount, "test": test_case["className"] + "." + test_case["name"], "bug_status": bug_status})
        if not failed_tests_amount:
            failed_tests.append({"test": "All Passed"})
        failed_tests_table[data[job]["name"]] = ({"name": data[job]["name"], "failed_tests": failed_tests, "fixed": fixed_test_cases, "regressed": regressed_test_cases})
    ...
    for topo in data:
        ...
        try:
            ...
            if data[topo]["regress"]["failures_regression"] > 0:
                data["total"]["regress"]["failures_regression"]["pos"] += data[topo]["regress"]["failures_regression"]
            else:
                data["total"]["regress"]["failures_regression"]["neg"] -= data[topo]["regress"]["failures_regression"]
            if data[topo]["regress"]["passed_regression"] > 0:
                data["total"]["regress"]["passed_regression"]["pos"] += data[topo]["regress"]["passed_regression"]
            else:
                data["total"]["regress"]["passed_regression"]["neg"] -= data[topo]["regress"]["passed_regression"]
            if data[topo]["regress"]["skipped_regression"] > 0:
                data["total"]["regress"]["skipped_regression"]["pos"] += data[topo]["regress"]["skipped_regression"]
            else:
                data["total"]["regress"]["skipped_regression"]["neg"] -= data[topo]["regress"]["skipped_regression"]
            if data[topo]["regress"]["total_regression"] > 0:
                data["total"]["regress"]["total_regression"]["pos"] += data[topo]["regress"]["total_regression"]
            else:
                data["total"]["regress"]["total_regression"]["neg"] -= data[topo]["regress"]["total_regression"]
...
def process_current_builds():
    data = {}
    total = {"name": "Total Result", "failures_number": 0, "passes_number": 0, "skipped_number": 0, "tests_number": 0,
             "results_link": os.environ["JENKINS_URL"], "data_link": os.environ["JENKINS_URL"], "time_str": "n/a", "total_time_str": "n/a"}
    ...
    for job in jobs:
        ...
        current_job = os.environ["JENKINS_URL"] + "job/" + TOPOS[topo]["job"] + "/"
        current_build_no = str(json.loads(requests.get(current_job + "api/json").content)["lastCompletedBuild"]["number"])
        if "TRIGGERED_BUILD_NUMBER_" + TOPOS[topo]["job"] in os.environ:
            current_build_no = os.environ["TRIGGERED_BUILD_NUMBER_" + TOPOS[topo]["job"]]
        current_link = (current_job + current_build_no + "/testReport/api/json?pretty=true")
        current_build_link = (current_job + current_build_no + "/api/json?pretty=true")
        try:
        ...
        try:
            ...
            data[topo].update({"results_link": current_job + current_build_no + "/testReport/"})
            back_iter = 1
            while json.loads(requests.get(current_job + str(int(current_build_no)-back_iter) + "/api/json?pretty=true").content)["result"] == "FAILURE" and back_iter < 4:
                back_iter += 1
            data[topo].update({"prev_build_link": current_job + str(int(current_build_no)-back_iter) + "/testReport"
                                                                                               "/api/json?pretty=true"})
            data[topo].update({"data_link": LOG_SERVER_LOCATION + TOPOS[topo]["job"] + "/" + current_build_no})
            data[topo].update({"current_build_info": current_job + current_build_no + "/api/json?pretty=true"})
            ...
            data[topo].update({"total_time": int(build_result['duration']) / 1000})
            data[topo].update({"total_time_str": str_time(int(build_result['duration']) / 1000)})
    total_json = json.loads(requests.get(os.environ["JENKINS_URL"] + "job/" + AGGREGATOR_JOB_NAME + "/lastCompletedBuild/api/json/").content)
...
def pretty_report(data):
    ...
    for topo in data:
        request = requests.get(data[topo]["data_link"])
        if 'results_link' in data[topo] and topo != "total" and request.status_code == 200:
            topos_template += table_row_template.format(
                **data[topo])
        elif 'results_link' in data[topo] and topo != "total" and request.status_code != 200:
...
def pretty_report_for_mail(data, bug_lists):
    ...
    <strong>Functional Product regression:               </strong> </br>
    ...
    <tr><a href="http://wikicentral.cisco.com/display/OPENSTACK/Nightly+testing"><strong>[Nightly Testing Details]</strong></a>
 </br> </br>"""
    ...
    for name, repo in GITHUB_URLS.iteritems():
        ...
        main_template += """
        <a href='{repo}/commit/{revision}'>{repo}</a></br> """.format(repo=repo["url"], revision=revision)
        ...
        for name, bug_list in bug_lists.iteritems():
            ...
            for bug in bug_list["failed_tests"]:
                ...
                if bug.get("test", "") == "All Passed":
                    ...
                    color = 'red'
                bug_table += table_row_bug_template.format(testNo=bug.get("test_no", "  "), testName=bug.get("test", " - "), color=color, color2=color2, bugNo=bug.get("bug_status", " - "))
            if bug_list["failed_tests"][0]["test"] != "All Passed":
                main_template += table_bug_template.format(job=name, rows=bug_table)
            else:
