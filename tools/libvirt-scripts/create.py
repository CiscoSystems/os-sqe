...
## TODO: remove GLOBALS
IMG_PATH = IMG_PATH_LOCAL=os.path.join(os.environ["HOME"], "hdd", "tmpdir","tmp", "imgs")
name_woext = "trusty-server-cloudimg-amd64-disk1"
ubuntu_img_path_local=os.path.join(os.environ["HOME"], "hdd", "tmpdir", name_woext + ".img")
EXTERNAL_NET = "public"
...
def main_disk_create(name, size, config, boot_type="net", user_seed_yaml=None, img_uncomp_path=None):
    ...
    if boot_type == "net":
        ...
        with open("/tmp/tmp_vol.xml", "w") as f:
            f.write(vol_xml)
        run_cmd(["virsh", "vol-create",  "--pool", "default", "--file" ,"/tmp/tmp_vol.xml"])
        ...
        user_seed_yaml_path = os.path.join("/tmp/", "tmp-yaml")
        with open(user_seed_yaml_path, "w") as f:
            f.write("#cloud-config\n" + user_seed_yaml)
        run_cmd(["qemu-img", "create", "-f", "qcow2", "-b", img_uncomp_path, path, "%sG" % size])
        run_cmd(["cloud-localds", seed_disk, user_seed_yaml_path])
...
def create_host_seed_yaml(config, box, btype):
        ...
        if net != EXTERNAL_NET:
            ints.append([
                config['params']['static_interface_template'].format(
                    ...
                    net_ip=networks[net]['net-ip'],
                    int_ip=networks[net]['net-ip'] + "." + last_octet,
                    dns="8.8.8.8"),
    ...
    user_seed_yaml['write_files'].append({
                "content": box["hostname"],
                "path": "/etc/hostname",
                })
    full_cmd = []
    for cmd in user_seed_yaml['runcmd']:
        if "hostname" in cmd:
            full_cmd.append(["hostname " + box["hostname"]])
        elif "ifdown" in cmd:
            for interf in ints:
                int_name = interf[2]
                full_cmd.append(["ifdown {int} && ifup {int}".format(int=int_name)])
        else:
            full_cmd.append(cmd)
...
def main():
    ...
    parser.add_argument('-d', action='store', dest='img_dir', default=None,
                        help='Where to store all images and disks, full abs path')
    parser.add_argument('-z', action='store', dest='ubuntu_img_dir', default=None,
    ...
    IMG_PATH = opts.img_dir or IMG_PATH_LOCAL
    ubuntu_img_path = opts.ubuntu_img_dir or ubuntu_img_path_local
    ...
    mac = rand_mac()
    ip = networks["boot"]["net-ip"] + "." + str(IP_START)
    ...
    for i in xrange(control_servers_len):
        mac = rand_mac()
        ip = networks["boot"]["net-ip"] + "." + str(IP_START + i + 1)
    ...
    for n in networks.itervalues():
        try:
            net_undefine(conn, n['name'])
        except:
            print "not found net", n['name']
    ...
    remove_all_imgs()
    if opts.boot == "net":
        if pool_found(conn, "default"):
            pool_delete(conn, "default")
        pool_create(conn, conf['params']['pool']['xml'].format(
            name="default", path=IMG_PATH))



    new_img_path = os.path.join(IMG_PATH, "lab-backing.img")
    convert_image(ubuntu_img_path, new_img_path)

    # create build server
    yaml_config = create_seed_yaml(conf, box=lab_boxes["build-server"][0]["hostname"])
    disk=main_disk_create(lab_boxes["build-server"][0]["name"],
                          10, conf, boot_type=opts.boot,
    ...
    vm_xml = conf["params"]["build-server.xml"].format(
        ...
        ram=4*1024*1024,
        disk=disk,
        net_boot_name=conf["params"]["networks"]["boot"]["name"],
        build_server_mac=lab_boxes["build-server"][0]["mac"],
        net_admin_name=conf["params"]["networks"]["admin"]["name"],
    )
    ...
    for compute in lab_boxes["compute-servers"]:
        ...
        disk = main_disk_create(compute["name"],
            10, conf, boot_type=opts.boot,
            user_seed_yaml=yaml_config,
            img_uncomp_path=new_img_path)
        vm_xml = conf["params"]["compute-server.xml"].format(
            ...
            mac=compute["mac"],
            net_admin_name=conf["params"]["networks"]["admin"]["name"],
            net_public_name=conf["params"]["networks"]["public"]["name"],
            net_internal_name=conf["params"]["networks"]["internal"]["name"],
            net_external_name=conf["params"]["networks"]["external"]["name"],
        )
