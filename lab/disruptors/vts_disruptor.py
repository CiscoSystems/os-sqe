...
class VtsDisruptor(ParallelWorker):
    ...
    def loop_worker(self, **kwargs):
        ...
        if 'vtc' in kwargs['node-to-disrupt']:
            ...
            for vtc in self.get_lab().get_nodes_by_class(Vtc):
                if vtc.get_nic('a').get_ip_and_mask()[0] == cluster[node_role]:
                    ...
                    break
        elif 'dl' in kwargs['node-to-disrupt']:
            master_id = None
        ...
        node_object_disrupted = kwargs.get('previously_disrupted', None)
        ...
        node_object_to_disrupt.disrupt(method_to_disrupt=kwargs['method-to-disrupt'], downtime=kwargs['downtime'])
        self.log('Sleeping for {} secs uptime'.format(kwargs['uptime']))
        time.sleep(kwargs['uptime'])

        kwargs['previously_disrupted'] = node_object_to_disrupt
