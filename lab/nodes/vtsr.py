...
class Vtsr(VirtualServer):
    ...
    def disrupt(self, method_to_disrupt, downtime):
        ...
        if method_to_disrupt == 'vm-shutdown':
            ...
            vts_host.exe(command='virsh suspend xrnc{}'.format(self.id[-1]))
            time.sleep(downtime)
            vts_host.exe(command='virsh resume xrnc{}'.format(self.id[-1]))
            ...
            # self.get_id()[-1] if id is "xrnc1" => 1, "xrnc2" => 2
            ans = vts_host.exe('ip l | grep mgmt | grep xrnc{}'.format(self.id[-1]))
            ...
            # self.get_id()[-1] if id is "xrnc1" => 1, "xrnc2" => 2
            ans = vts_host.exe('ip l | grep tenant | xrnc{}'.format(self.id[-1]))
            if_name = ans.split()[1][:-1]
            vts_host.exe('ip l s dev {} down'.format(if_name))
            time.sleep(downtime)
            vts_host.exe('ip l s dev {} up'.format(if_name))
...
'''
        ...
        file_name = 'expect-{}-{}'.format(self.id, cmd_name)
...
''')
    ...
    def get_config_and_net_part_bodies(self):
        ...
        dns_ip, ntp_ip = self.pod.dns[0], self.pod.ntp[0]
        xrvr_name = self.id
        xrnc_name = xrvr_name.replace('xrvr', 'xrnc')

        _, vtc_username, vtc_password = self.pod.get_node_by_id('vtc1').get_oob()
        ...
        xrvr_mx_ip = mx_nic.get_net().get_ip_for_index(200 + int(self.id[-1]))
        ...
        xrvr_te_ip = te_nic.get_net().get_ip_for_index(200 + int(self.id[-1]))
    ...
    def r_xrnc_start_dl(self):
        ...
        ips = [x.get_nic('t').get_ip_and_mask()[0] for x in self.pod.get_nodes_by_class(Vtsr)]
    ...
    def r_edit_etc_hosts(self):
        self.exe('grep {n} /etc/hosts || echo {n}\t{ip}\n >> /etc/hosts'.format(n=self.id, ip=self.get_ip_mx()))
