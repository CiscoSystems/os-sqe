...
class MercuryMgm(CimcServer):
    ...
    def r_create_sqe_user(self):
        ...
        if not self.exe(cmd='grep {} /etc/passwd'.format(self.pod.SQE_USERNAME), is_warn_only=True):
            ...
            self.exe(cmd='adduser -p ' + encrypted_password + ' ' + self.pod.SQE_USERNAME)
            self.exe(cmd='echo "{0} ALL=(root) NOPASSWD:ALL" | tee -a /etc/sudoers.d/{0}'.format(self.pod.SQE_USERNAME))
            self.exe(cmd='chmod 0440 /etc/sudoers.d/' + self.pod.SQE_USERNAME)

            server = Server(ip=self.ssh_ip, username=self.pod.SQE_USERNAME, password=tmp_password)
            server.exe('mkdir -p .ssh && chmod 700 .ssh')
            server.exe('echo "{}" > .ssh/id_rsa && chmod 600 .ssh/id_rsa'.format(self.pod.PRIVATE_KEY))
            server.exe('echo "{}" > .ssh/id_rsa.pub && cp .ssh/id_rsa.pub .ssh/authorized_keys && chmod 600 .ssh/authorized_keys'.format(self.pod.PUBLIC_KEY))
